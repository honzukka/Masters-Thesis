...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        %\label{fig:background_light_transport_examples-procam}
    \end{subfigure}
    \caption{An illustration of the difference between rendering (left) and projection mapping (right). Recall fig.~\ref{fig:intro_procam} that we build on here. When rendering, light sources (here projection \(\bm{p}\)), materials and geometry are known and radiance at the camera sensor (here camera image \(r(\bm{p})\) is unknown). On the other hand, in a projection mapping scenario we know only what our camera image should look like (\(\bm{y}\)) and sometimes we know the relative position of projector and camera. Everything else is unknown}
