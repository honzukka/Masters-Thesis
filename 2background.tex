...
The problem of projection mapping is usually split into two parts: \textit{geometric calibration} and \textit{radiometric calibration}. The goal of geometric calibration is to determine correspondencies between projector pixels and camera pixels in a procam. Once we know which projector pixels affect which camera pixels, and therefore the pixels of our desired appearance, we need to find our \textit{how} they affect them. This is done using radiometric calibration which determines the relationship between the intensities of each pair of corresponding projector and camera pixels.
...
We will now briefly cover a few of the latest methods for geometric and radiometric calibration in order to get an idea about the way they approach the complex general problem of inverse light transport described in \ref{section:background-projection_mapping-light_transport-towards_projection_mapping}. For a complete overview of the state of the art in projection mapping, see \citet{Grundhofer2018}.
