...
{\color{red} TODO: in figures, highlight what the final output is. now it blends in with the baseline too much}

{\color{red} TODO: ask Jaakko if we should comment on the fact that the optimizer pushes pixel values all the way to max (mostly in the staircase scene)?}
...
\begin{figure}[]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \end{subfigure}
    \end{subfigure}
    \caption{{\color{red} TODO: what's our optimized result? individual image labels should be clear even without reading figure caption or text around it!} Results of experiment 2. We use the simple rendering function (see Section~\ref{section:methods-rendering_function-simple}) and compare our method with the original Gatys texture model (see Section~\ref{section:methods-texture_model}) against our method with the improved texture model (specifically, the Pyramid + Shift variant from Section~\ref{section:methods-texture_model-improvements}) and a conventional pixel-based projection mapping method. Brightness \(c\) is set to \(5\) to adjust the difficulty of the scene. Note how our method with the improved model is better than Gatys at matching the large features of the input (e.g. the outline of the woman is not visible and the size of the marble grain stays roughly the same). It is also better than the pixel-based method at matching the colors (e.g. the blond hair is not visible in the final camera image). {\color{red} TODO: show more results here, so that it's possible to see the relative performance and draw better conclusions} Texture source: \citet{Pixar128}}
...
\begin{figure}[]
    ...
    \begin{subfigure}{0.7\textwidth}
        ...
        \begin{subfigure}{0.48\textwidth}
            \centering
            \includegraphics[width=\textwidth]{images/04-experiment02/human/marble/gatys_proj_highlighted.jpg}
            \caption*{}
    ...
    \begin{subfigure}{0.7\textwidth}
        ...
        \begin{subfigure}{0.48\textwidth}
            \centering
            \includegraphics[width=\textwidth]{images/04-experiment02/human/marble/improved_proj_highlighted.jpg}
            \caption*{}
            %\label{fig:ex02-human-marble-detail-highlighted}
        \end{subfigure}
        \hfill
    ...
    \begin{subfigure}{0.7\textwidth}
        ...
        \begin{subfigure}{0.48\textwidth}
            \centering
            \includegraphics[width=\textwidth]{images/04-experiment02/human/marble/pixel_proj_highlighted.jpg}
            \caption*{}
            %\label{fig:ex02-human-marble-detail-highlighted}
        \end{subfigure}
        \hfill
        ...
        %\label{fig:ex02-human-marble-detail-crops}
    \end{subfigure}
    \caption{{\color{red} TODO: label + caption + reference from text} Texture source: \citet{Pixar128}}
...
\begin{figure}[]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \end{subfigure}
    \end{subfigure}
    \caption{{\color{red} TODO: still a bit hard to parse. emphasize that the loss is only computed in the cropped region!} Results of experiment 3. We use the general rendering function (see section ~\ref{section:methods-rendering_function-general}) and compare our method with the improved texture model (see Section~\ref{section:methods-texture_model-improvements}; marked as "O.") against a pixel-based reference (marked as "P."). \(c = 5\). The scene contains a mirror ball which is being projected onto from the ceiling as suggested in image~\ref{fig:ex03-ball_dof-scene}. The projector has depth of field enabled and the camera image is cropped (see the scissors symbol) before computing the loss to match the input texture image. Overall, our pipeline handles this complex light transport well. Note how the box is missing one wall (the black patch inside the mirror ball) which causes the optimization to leave the corresponding part of the compensation image untouched. Texture source: \citet{Pixar128}}
...
\begin{figure}[]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \begin{subfigure}{0.5\textwidth}
            ...
            \includegraphics[width=\textwidth]{images/04-experiment03/staircase_illum/scene_highlighted.jpg}
            \caption{Scene}
        ...
        \end{subfigure}
    \end{subfigure}
    \caption{{\color{red} TODO: what's up with the red rectangle? :D} Results of experiment 3. We use the general rendering function (see section ~\ref{section:methods-rendering_function-general}) and compare our method with the improved texture model (see Section~\ref{section:methods-texture_model-improvements}; marked as "O.") against a pixel-based reference (marked as "P."). \(c = 5\). The scene contains a wooden staircase which is being projected onto from the point of view of the camera as suggested in image~\ref{fig:ex03-staircase_illum-scene}. The projector has depth of field disabled and the camera image is not cropped before computing the loss. Note how the performance of our method corresponds to previous experiments. An example is highlighted by green ellipses. The glossy wooden floor in the lower left corner of the scene is difficult to project onto. The dark patch of the input texture (highlighted) is easy to project onto the corner because it already looks like it. This is what our method does (see image~\ref{fig:ex03-staircase_illum-beams-stats_proj}). On the other hand, the pixel-based reference cannot do this and instead oversaturates corresponding projector image pixels (see image ~\ref{fig:ex03-staircase_illum-beams-pixel_im}). Texture source: \citet{Pixar128}}
